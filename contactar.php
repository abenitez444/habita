...
$myrow = $result5->fetch_assoc();
 ...
 $mail = $myrow["cat_desc"];
	...
	$msg .= "Ha recibido comentarios desde Avita \n";
	...
	// mail($receiverMail, "Contacto desde Avita", $msg, $mailheaders);
	
	$response = '';
	
	$mailSMTP->ClearAllRecipients( );
	$mailSMTP->AddAddress($receiverMail);
	$mailSMTP->Subject = 'Contacto desde Avita';
	$mailSMTP->Body    = $msg;
	
	if ($mailSMTP->Send()) {
        $response = 'Gracias por ponerte en contacto con nosotros '.$nombre.'.';
    } else {
        $response = "Message could not be sent. Mailer Error: {$mailSMTP->ErrorInfo}";
    }   

?>
 </p>
  <p><?php echo $response?></p>&nbsp;</td>
